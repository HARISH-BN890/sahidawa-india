...
export default function ExpiryTrackerPage() {
    ...
    const handleExportCSV = () => {
        ...
        const generatedOn = `"${t("generatedOn")}: ${new Date().toLocaleDateString()}"`;
        const header = ["Medicine Name", "Expiry Date", "Batch Number", "Status"];
        const rows = processedMedicines.map((med) => {
            ...
            return [
                ...
                `"${(med.batchNumber ?? "").replace(/"/g, '""')}"`,
                status.text,
            ].join(",");
        });
        const csv = [generatedOn, header.join(","), ...rows].join("\n");
        const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
        const url = URL.createObjectURL(blob);
        const a = document.createElement("a");
        a.href = url;
        a.download = "sahidawa_expiry_tracker.csv";
        a.click();
        URL.revokeObjectURL(url);
        showToast(t("csvExportSuccess"));
    };

    // Export PDF via jsPDF (dynamic import; add jspdf + jspdf-autotable to package.json if missing)
    const handleExportPDF = async () => {
        ...
        try {
            // Use autotable for clean tables with proper column wrapping
            ...
            // Fallback: manual table if autotable not installed
            let y = 36;
        ...
        <>
            ...
            <style>{`
                ...
                @media print {
                    ...
                    .print-table th, .print-table td { border: 1px solid #ccc; padding: 8px 12px; text-align: left; font-size: 13px; }
            ...
            <div className="print-only" style={{ fontFamily: "sans-serif", padding: "24px" }}>
                <h1 style={{ marginBottom: 4 }}>SahiDawa — Medicine Expiry Tracker</h1>
                <p style={{ fontSize: 12, marginBottom: 16 }}>{new Date().toLocaleDateString()}</p>
                {processedMedicines.length === 0 ? (
                    <p>{t("noMedicinesToDisplay")}</p>
                ) : (
                    <table className="print-table">
                        ...
                        <tbody>
                            {processedMedicines.map((med) => (
                                <tr key={med.id}>
                                    <td>{med.name}</td>
                                    <td>{parseLocalDate(med.expiryDate).toLocaleDateString()}</td>
                                    <td>{med.batchNumber || "—"}</td>
                                    <td>{getExpiryStatus(med.expiryDate).text}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
            ...
            <div className="no-print min-h-screen bg-(--color-surface-page) text-(--color-text-primary) transition-colors duration-300">
                {toast && (
                        ...
                        <div className="h-fit rounded-2xl border border-(--color-border-muted) bg-(--color-surface-muted) p-6 shadow-sm md:sticky md:top-32 md:col-span-1">
                            ...
                            <div className="mt-6 flex flex-col gap-2">
                                ...
                                <button
                                    ...
                                    disabled={processedMedicines.length === 0}
                                            ...
                                            >
                                                ...
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <button
                                                        ...
                                                        aria-label={`${t("deleteMedicine")} ${med.name}`}
